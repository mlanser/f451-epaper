...
def main():
    ...
    try:
        ...
        logging.info("Custom f451 Labs images")
        HBlackimage = Image.open(os.path.join(picdir, 'f451-piSkyLab-black.bmp'))
        HRedimage = Image.open(os.path.join(picdir, 'f451-piSkyLab-red.bmp'))
        epd.display(epd.getbuffer(HBlackimage), epd.getbuffer(HRedimage))
        time.sleep(5)
